...
class EmploiController {
    ...
    static create = async (req, res, next) => {
        const { name, classeId, adminId, startDate, endDate } = req.body;
        const emploi = {
            name,
            classeId,
            adminId,
    ...
    static update = async (req, res, next) => {
        ...
        try {
            ...
            return res.status(204).json({
                success: true,
                message: `Emploi ${id} updated correctly`,
            });
        } catch (error) {
            next(error);
        }
    ...
    static delete = async (req, res, next) => {
        ...
        try {
            ...
            return res
                .status(200)
                .json({ success: true, message: `Emploi ${id} deleted` });
        } catch (err) {
            next(err);
        }
    };
    static async generateEmploisStudent(req, res, next) {
        try {
            ...
            return res
                .status(200)
                .json({ success: true, message: `Emploi generated` });
        } catch (err) {
            next(err);
        }
    }
