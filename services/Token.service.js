...
class Token {
    static createToken(user, options) {
        ...
        if (options.type === "access") {
            ...
            expiresIn = "15m";
            payload = { userId: user.id, accountType: user.accountType };
        }
    ...
    static async refreshToken(req, res) {
        ...
        return res.send({
            ok: "true",
            accessToken: Token.createToken(user, { type: "access" }),
        });
    }
    ...
    static async revokeRefreshTokens(userId) {
        ...
        if (!updatedUser) {
            throw ErrorResponse.notFound(
                "could not find the user to revoke the token"
            );
        }
